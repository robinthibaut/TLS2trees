...
def run_training(params):
    ...
    if params.verbose:
        print(
            f"Running deep learning using {params.dl_cpu_cores}/{os.cpu_count()} CPU cores"
        )
    ...
    if os.path.isfile(params.model):
        ...
        model.load_state_dict(
            torch.load(params.model), strict=False
        )  # load exiting model
        ...
        params.train_history = pd.DataFrame(
            columns=[
                "epoch",
                "epoch_loss",
                "epoch_acc",
    ...
    optimizer = optim.Adam(
        filter(lambda p: p.requires_grad, model.parameters()), lr=params.learning_rate
    )
    ...
    for epoch in tqdm(range(params.iterations), total=params.iterations):
        ...
        for i, data in enumerate(train_loader):
            ...
            if i % 5 == 0:
                ...
                if params.generate_point_cloud_vis:
                    running_point_cloud_vis = pd.DataFrame(
                        running_point_cloud_vis,
                        columns=["x", "y", "z", "label", "pred"],
                    )
                    ply_io.write_ply(
                        f"{params.out}.{epoch}.ply", running_point_cloud_vis
                    )

        # write training history to file
        epoch_loss = running_loss / len(train_loader)
        epoch_acc = running_acc / len(train_loader)
        params.train_history.loc[len(params.train_history)] = [
            epoch,
            epoch_loss,
            epoch_acc,
            val_epoch_loss,
        ...
        params.train_history.to_csv(
            f"{params.out}.training_history.csv", header=True, index=False
        )
        if params.verbose:
            print(
                f"Train epoch accuracy: {np.around(epoch_acc, 4)} Loss: {np.around(epoch_loss, 4)}\n"
            )
        ...
        if params.validate:
            ...
            for data in validation_loader:
                ...
                if i % 5 == 0:
                    print(
                        f"Validation sample accuracy: {np.around(running_acc / (i + 1), 4)} Loss: {np.around(running_loss / (i + 1), 4)}"
                    )
            ...
            print(
                f"Validation epoch accuracy: {np.around(val_epoch_acc, 4)} Loss: {np.around(val_epoch_loss, 4)}"
            )
            print(
                "====================================================================="
            )
...
def preprocessing_setup(dataset, out_dir, params):
    ...
    if isinstance(dataset, str):
        if os.path.isfile(dataset):
            ...
            dataset = glob.glob(os.path.join(dataset, "*.ply"))
        else:
            raise Exception(f"{dataset} is not a .ply or a directory")
    ...
    if len(dataset) == 0:
        raise Exception(f"no .ply files in {dataset}")

    for pc in dataset:
        pc = ply_io.read_ply(pc)
        if params.label not in pc.columns:
            raise Exception(
                f'{params.label} not in pc fields, available columns are {", ".join([c for c in pc.columns[3:]])}'
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="path to training point cloud",
    )
    parser.add_argument(
        "--validate",
        "-v",
        ...
        default=False,
        type=str,
        help="path to validation point clouds",
    )
    parser.add_argument(
        "--chunks-dir", default="", type=str, help="path to chunks directory"
    )

    parser.add_argument(
        "--label", default="label", type=str, help='name of "label" field in input'
    )
    parser.add_argument(
        "--iterations",
        default=200,
    ...
    # model arguments
    parser.add_argument(
        "--model", "-m", type=str, required=True, help="output directory"
    )
    ...
    parser.add_argument(
        "--save-chunks-to",
        default=None,
        help="directory to save chunks to, otherwise saved to a temporaray directory",
    )
    ...
    if params.cpu_cores == 0:
        ...
        if params.verbose:
            print(f"Processing using {params.cpu_cores}/{os.cpu_count()} CPU cores")
    ...
    if params.chunks_dir != "":
        ...
        if not os.path.isdir(os.path.join(params.chunks_dir, "train")):
            raise Exception(
                f'no such directory: {os.path.join(params.chunks_dir, "train")}'
            )
        if not os.path.isdir(os.path.join(params.chunks_dir, "validate")):
            raise Exception(
                f'no such directory: {os.path.join(params.chunks_dir, "validate")}'
            )
        params.dtrain = os.path.join(params.chunks_dir, "train")
        params.dvalidate = os.path.join(params.chunks_dir, "validate")
        if len(glob.glob(os.path.join(params.dtrain, "*.npy"))) == 0:
            raise Exception(f"no .npy files in {params.train}")
        if len(glob.glob(os.path.join(params.dvalidate, "*.npy"))) == 0:
            raise Exception(f"no .npy files in {params.validate}")
