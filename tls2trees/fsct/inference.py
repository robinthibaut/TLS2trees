...
def SemanticSegmentation(params):
    ...
    if not np.all(
        np.isclose(
            params.pc.loc[~params.pc.buffer][["x", "y", "z"]].mean(),
            [0, 0, 0],
            atol=0.1,
        )
    ):
    ...
    test_loader = DataLoader(
        test_dataset, batch_size=params.batch_size, shuffle=False, num_workers=0
    )

    # initialise model
    model = Net(num_classes=4).to(params.device)
    model.load_state_dict(
        torch.load(params.model, map_location=params.device), strict=False
    )
    model.eval()
    ...
    params.pc = params.pc.drop(
        columns=[
            c
            for c in params.pc.columns
            if c in ["label", "pTerrain", "pLeaf", "pWood", "pCWD"]
        ]
    )
    ...
    params.pc.loc[params.pc.n_z <= params.ground_height_threshold, "label"] = (
    ...
    save_file(
        os.path.join(
            params.odir,
            "{}.segmented.{}".format(params.filename[:-4], params.output_fmt),
        ),
        params.pc.loc[~params.pc.buffer],
        additional_fields=["n_z", "label", "pTerrain", "pLeaf", "pCWD", "pWood"]
