...
def RANSACcylinderFitting4(xyz_, iterations=50, plot=False):
    ...
    for i in range(iterations):
        ...
        centre = (x, y)
        if not np.all(np.isclose(centre, 0, atol=radius * 1.05)):
            continue
        ...
        idx = xyz.loc[
            xyz.error.between(0.8, 1.2)
        ].index  # 40% of radius is prob quite large

        # select points which best fit model from original dataset
        alsoInliers = xyz_.loc[idx].copy()
        if len(alsoInliers) < len(xyz_) * 0.2:
            continue  # skip if no enough points chosen
        ...
        alsoInliers.loc[:, "error"] = (
            np.linalg.norm(alsoInliers[["x", "y"]] - centre, axis=1) / radius
        )
...
def NotRANSAC(xyz):
    ...
    try:
        ...
        if (
            xyz.z.min() - radius < centre[0] < xyz.z.max() + radius
            or xyz.y.min() - radius < centre[1] < xyz.y.max() + radius
        ):
...
def RANSAC_helper(xyz, ransac_iterations, plot=False):
    ...
    if len(xyz) == 0:  # don't think this is required but....
        ...
        cylinder = RANSACcylinderFitting4(
            xyz[["x", "y", "z"]], iterations=ransac_iterations, plot=plot
        )
